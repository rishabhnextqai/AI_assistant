...
1. **Introduction**:
   - Find the account name and the client name and start with "Welcome to Next Quarter's briefing on `account name`, prepared exclusively for `client name`."
...
def create_podcast(script: str) -> Optional[AudioSegment]:
    ...
    def process_line(line: str):
        if line.startswith("Host1:"):
            ...
            return text_to_speech_stream(line[6:].strip(), voice="shimmer")
        return None
    ...
    for i, (line, audio_content) in enumerate(zip(lines, audio_streams)):
        if audio_content:
            try:
                voice = "ash" if line.startswith("Host1:") else "shimmer"
