...
#     """Splits long text into safe chunks for OpenAI TTS input."""
#     words = text.split()
#     chunks = []
#     current_chunk = []

#     for word in words:
#         if sum(len(w) + 1 for w in current_chunk + [word]) <= max_length:
...
#     """Splits a large string into chunks of max_chars, preferably on paragraph boundaries."""
...
# def text_to_speech_stream(text: str, voice: str = "ash") -> Optional[bytes]:
#     """Converts input text into speech audio, handling chunking for long text."""
#     try:
#         logger.info(f"Starting TTS for voice: {voice}")
...
#     except Exception as e:
#         logger.error(f"TTS failed for voice {voice}: {str(e)}")
#         return None

# def create_podcast(script: str) -> Optional[AudioSegment]:
#     """Takes a full TED-style script and returns a combined audio segment."""
#     lines = [line.strip() for line in script.split('\n') if line.strip()]
#     audio_segments: List[AudioSegment] = []

#     def process_line(line: str) -> Optional[bytes]:
#         return text_to_speech_stream(line, voice="ash")

#     logger.info(f"Starting podcast generation for {len(lines)} lines")
...
#                 logger.info(f"Segment {i+1} processed")
#             except Exception as e:
#                 logger.error(f"Error loading audio segment {i+1}: {str(e)}")
#         else:
#             logger.warning(f"Skipping line {i+1} due to missing audio")

#     if audio_segments:
#         final_audio = sum(audio_segments)
#         logger.info(f"Combined audio duration: {len(final_audio) / 1000:.2f} seconds")
#         return final_audio
#     else:
...
# def generate_podcast_script_intelligence(file_contents: str, podcast: str, account_name: str, client_name: str) -> str:
#     def get_prompt(section: str, chunk: str, part: int) -> str:
#         base_prompt = {
#             'exec': f"""
# Create a dynamic and engaging TED Talk script for "Next Quarter's Executive Briefing" featuring a single speaker. The talk should focus on strategic priorities and key initiatives for {account_name}, using the provided content as the foundation:
...
# """,
#             'businessoverview': f"""
# Continue the TED Talk (part {part}) for {account_name}, focusing on:

# - SWOT Analysis (1 key strength, 1 weakness)
# - Roadmap highlights
# - Market + Industry trends
# - Use practical, sales-focused storytelling
# - End with a teaser for competitors section

# Content:
# {chunk}
# """,
#             'competitors': f"""
# Continue the TED Talk (part {part}) on {account_name}, focusing on:

# - Key competitors and positioning
# - 2 client gaps with fixes
# - Spotlight on 1 major competitor
# - Countermeasures
# - Tease upcoming stakeholder section

# Content:
# {chunk}
# """,
#             'stakeholders': f"""
# Conclude the TED Talk (part {part}) on {account_name}, focusing on:

# - Stakeholder personas and contributions
# - Key executive bios
# - Relationship-building strategies
# - End with thank you and motivational close
...
#     chunks = chunk_text(file_contents, max_chars=20000)
#     logger.info(f"[{podcast}] Intelligence script split into {len(chunks)} chunk(s)")

#     full_script = ""
#     for i, chunk in enumerate(chunks):
#         try:
#             prompt = get_prompt(podcast, chunk, part=i + 1)
#             logger.info(f"[{podcast}] Generating script chunk {i + 1}/{len(chunks)}")
...
#             part_script = response.choices[0].message.content.strip()
#             full_script += f"\n\n{part_script}"

#         except Exception as e:
#             logger.error(f"Error generating {podcast} script for chunk {i + 1}: {str(e)}")

#     return process_script(full_script.strip())


def chunk_text(text: str, max_chars: int = 20000) -> List[str]:
    """Split text into chunks at paragraph boundaries with safe token count."""
    paragraphs = text.split("\n\n")
    chunks = []
    current = ""
...
def generate_podcast_script_intelligence(file_contents: str, podcast, account_name, client_name) -> str:
    chunks = chunk_text(file_contents, max_chars=20000)
    logger.info(f"[{podcast}] Split document into {len(chunks)} chunk(s)")
...
def generate_podcast_script_ABM(file_contents: str, account_name, client_name) -> str:
    chunks = chunk_text(file_contents, max_chars=20000)
    logger.info(f"[ABM] ABM script split into {len(chunks)} chunk(s)")
    full_script = ""

    for i, chunk in enumerate(chunks):
        logger.info(f"[ABM] Generating ABM script chunk {i + 1}/{len(chunks)}")
...
"""
        ...
        try:
            ...
            logger.error(f"Error generating ABM script for chunk {i + 1}: {str(e)}")
