...
def generate_podcast_script(file_contents: str, podcast) -> str:
    if podcast == 'exec':
        ...
        1.Start with a brief and upbeat introduction to the podcast's purpose (skip detailed intros of Ethan and Zoe).

        2. Select the top 3 initiatives from the initiatives section and for each:
        - Summarize each initiative briefly before diving into the recommended alignment.
        - Highlight one relevant case study for each initiative.
        - Limit the discussion of each initiative to about 130 words.

        3. Summarize other sections apart from the initiatives from the perspective of a sales executive selling into the account.
        ...
        Zoe: [Ethan's dialogue]

        Ethan: [Zoe's dialogue]
...
def create_podcast(script: str) -> Optional[AudioSegment]:
    ...
    def process_line(line: str):
        if line.startswith("Ethan:"):
            ...
            return text_to_speech_stream(line[6:].strip(), voice="sage")
        return None
    ...
    for i, (line, audio_content) in enumerate(zip(lines, audio_streams)):
        if audio_content:
            try:
                voice = "ash" if line.startswith("Ethan:") else "sage"
